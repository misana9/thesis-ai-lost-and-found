...
A **local** software solution is still required. Philippine campus platforms typically digitize reporting and admin queues yet leave comparison to keyword search or staff pairing (Castro et al., 2022). Stock (pretrained) CLIP is a strong starting encoder, but campus phone photos and short student descriptions form a narrower domain than CLIP’s web-scale training distribution: lighting, camera quality, look-alike gadgets, and vague local wording create a domain gap. AMAlost therefore starts from CLIP ViT-B/32 and applies **light campus fine-tuning** on a curated Objects photograph set so embeddings better reflect institutional items—without training a vision–language model from scratch. The study further treats evaluation as local and controlled: improvements are demonstrated on the researcher’s curated lost-and-found Objects database by benchmarking traditional keyword matching against multimodal CLIP-based ranking, rather than by importing published scores from other campuses.

Accordingly, this study develops **AMAlost**, a web-based matching system that encodes lost and found submissions with CLIP ViT-B/32 (optionally campus-adapted), ranks candidate pairs through multimodal semantic similarity, and connects acceptance to staff-mediated desk custody before final release. The remainder of Chapter 1 presents the statement of the problem, objectives, significance, scope and limitations, hypotheses, theoretical and conceptual frameworks, and definition of terms. Related literature is reviewed in Chapter 2, while technical requirements and architecture are presented in Chapter 3. The expected contribution is a working matching pipeline and workflow that can improve the speed and structure of institutional item recovery while keeping final handoff under campus staff control.
...
2. How can visual and textual features of lost and found submissions be represented as embeddings (including light campus fine-tuning of CLIP) so that cross-modal comparison becomes possible, and how does that multimodal approach improve on traditional keyword matching on the curated Objects dataset (precision, recall, false positive rate, Recall@k / MRR)?
...
4. How can ranked matching results be connected to a confirmation, notification, and staff desk-custody workflow that supports coordinated recovery without silently linking incorrect items?
...
1. To design and implement a ticket-based web reporting system that accepts lost and found submissions with image and/or text inputs, category, location, date, contact email, and optional serial numbers / distinctive marks for high-value items, and stores those tickets for retrieval.

2. To generate normalized 512-dimensional image and text embeddings for each submission using CLIP ViT-B/32, with light contrastive fine-tuning on campus Objects photographs (item-level 80/20 split), persist the vectors in PostgreSQL with pgvector, and demonstrate improvement of this multimodal embedding/search approach over traditional keyword matching on the curated Objects dataset (precision, recall, false positive rate, Recall@k / MRR).
...
4. To integrate bidirectional matching (lost-to-found and found-to-lost) with a confirmation-centered claim workflow, email notifications, and staff desk custody (receive/release) after a match is accepted—including optional serial numbers and distinctive marks for high-value items.
...
The study covers the design and development of AMAlost as an institutional lost-and-found matching prototype. In terms of locality and users, the system is intended for campus stakeholders—students, faculty, and staff—who can access a web interface over an internet connection. In terms of technical focus, the investigation centers on ticket intake, CLIP ViT-B/32 embedding generation (pretrained plus light campus fine-tuning), multimodal semantic similarity ranking, ranked shortlist presentation, claim confirmation, staff desk custody, optional serial/distinctive-mark fields for high-value items, and claim-related email coordination through the campus lost-and-found desk. Evaluation within the design project emphasizes functional behavior of these components, keyword-versus-multimodal comparison on the curated Objects set, and ranking quality under controlled test conditions.
...
2. **Encoder and look-alike limits.** The prototype starts from pretrained CLIP ViT-B/32 and applies light campus fine-tuning on a limited curated Objects set (order of ~349 images / ~46 item identities) under an item-level 80/20 train–held-out split. Even after adaptation, performance remains bounded by image/text quality, look-alike items, and that modest labeled scale; the study does not claim web-scale retraining of CLIP.
